...
namespace Softeq.NetKit.Components.EventBus.Service
{
	public class EventBusService : IEventBusPublisher, IEventBusSubscriber
	{
		...
		private async Task RemoveDefaultRuleIfExists()
		{
			...
			{
				throw new Exceptions.ServiceBusException($"Removing default rule {RuleDescription.DefaultRuleName} (if exists) failed.", ex.InnerException);
			}
		}
		...
		private async Task AddSubscriptionRuleIfNotExists(string eventName)
		{
			...
			{
				throw new Exceptions.ServiceBusException($"Adding subscription rule (if not exists) for the entity {eventName} failed.", ex.InnerException);
			}
		}
